...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row data-cy-name={name}>{name}</Row>
      <ReactPlayer data-cy-get={youtubeLink} url={youtubeLink} width="100%" height="100%" />
      <Row data-cy-score={`${name}-${score}`}>
        <GoArrowUp size="24px" data-cy-upvote={`${name}-upvote`} onClick={handleUpvote} />
        {score}
        <GoArrowDown size="24px" data-cy-downvote={`${name}-downvote`} onClick={handleDownvote} />
      </Row>
    </Container>
  );
}
